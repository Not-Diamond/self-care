...
node "${CLAUDE_PLUGIN_ROOT}/lib/autosync.mjs" tick --config "$CONFIG_PATH" --state "$STATE_PATH"
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/autosync.mjs" complete --state "$STATE_PATH" --trace-hash "<trace_hash>"
```
