...
// packages/core/src/trace-importer/cli/list.ts
...
async function main() {
  ...
  if (!source || !["langsmith", "langfuse"].includes(source)) {
    console.error("Usage: list.ts --source langsmith|langfuse [--limit N] [--cursor str] [--page N] [--name str] [--status str] [--tag str]");
    process.exit(1);
  }
  if (source === "langsmith") {
    ...
    if (!project) {
      console.error("--project or --config with langsmith.project required.");
      process.exit(1);
    }
    ...
    const result = await client.listRuns(apiKey, project, limit, args.cursor, Object.keys(filters).length > 0 ? filters : void 0);
  ...
  if (source === "langfuse") {
    ...
    if (configPath) {
      const config = loadConfig(configPath);
      if (!projectId) projectId = config?.langfuse?.project_id ?? "";
      configHost = config?.langfuse?.host;
    }
    const host = args.host || process.env.LANGFUSE_HOST || process.env.LANGFUSE_BASE_URL || configHost;
    const client = new LangfuseClient();
    const result = await client.listTraces(publicKey, secretKey, host, page, limit, projectId || void 0);
    let traces = result.traces;
    const hasClientFilter = !!(args.name || args.tag);
    if (args.name) {
      const search = args.name.toLowerCase();
      traces = traces.filter((t) => t.name.toLowerCase().includes(search));
    }
    if (args.tag) {
      traces = traces.filter((t) => t.tags?.includes(args.tag));
    }
    const hasMoreServerPages = result.traces.length >= limit;
    console.log(JSON.stringify({
      traces: traces.map((t) => ({
        id: t.trace_id,
        name: t.name,
      ...
      pagination: {
        next_cursor: null,
        next_page: hasMoreServerPages ? page + 1 : null
      }
    }, null, 2));
  }
}
