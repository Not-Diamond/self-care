...
// packages/core/src/trace-importer/cli/fetch.ts
...
async function main() {
  ...
  const id = args.id;
  if (!source || !id) {
    console.error("Usage: fetch.ts --source langsmith|langfuse --id <id> [--project <name>] [--host <url>] [--project-id <id>]");
    process.exit(1);
  }
  if (source !== "langsmith" && source !== "langfuse") {
    console.error(`Unknown source: "${source}". Must be langsmith or langfuse.`);
    process.exit(1);
  }
  if (source === "langsmith") {
    ...
    if (!project) {
      console.error("--project is required for langsmith.");
      process.exit(1);
    }
    const client = new LangSmithClient();
    const projectInfo = await client.validate(apiKey, project);
    const runs = await client.fetchTraceRuns(apiKey, id);
    if (runs.length === 0) {
      console.error(`No runs found for trace ID: ${id}`);
      process.exit(1);
    }
    const otelJson = convertRunsToOtel(runs);
    const sourceUrl = projectInfo.tenant_id ? `https://smith.langchain.com/o/${projectInfo.tenant_id}/projects/p/${projectInfo.id}/r/${id}` : null;
    console.log(JSON.stringify({ source_url: sourceUrl, otel: otelJson }));
  }
  if (source === "langfuse") {
    ...
    const trace = await client.fetchTrace(publicKey, secretKey, id, host);
    if (!trace.observations || trace.observations.length === 0) {
      console.error(`No observations found for trace ID: ${id}`);
      process.exit(1);
    }
    const otelJson = convertObservationsToOtel(trace.id, trace.name, trace.observations);
    const baseUrl = (host || "https://cloud.langfuse.com").replace(/\/+$/, "");
    const sourceUrl = projectId ? `${baseUrl}/project/${projectId}/traces/${id}` : `${baseUrl}/traces/${id}`;
    console.log(JSON.stringify({ source_url: sourceUrl, otel: otelJson }));
  }
}
main().catch((err) => {
